...
<?php
$product_array = $db_handle->runQuery("SELECT * FROM project WHERE email ='".$id."' ORDER BY id DESC");
...
<br>
<div id="<?php echo $product_array[$key]["id"]; ?>b" style="display:j"><button style="float:left;background-color:green;border:1px solid green;padding:15px 15px;border-radius:5px"> Work Status : we will verify the bidding person & and show best three bids for you </button><button value="<?php echo $product_array[$key]["id"]; ?>" onclick="hi(this)" style="float:right;background-color:green;border:1px solid green;padding:15px 15px;border-radius:5px"> See Project Bids </button> </div> 
...
<?php
  $product_array = $db_handle->runQuery("SELECT * FROM bids WHERE p2 ='".$id."' AND p1='".$product_array[$key]["value"]."'");
	if (!empty($product_array)) { 
	foreach($product_array as $key=>$value){
    if($product_array[$key]["active"] == '6') {
?>

<?php

	}
	else if($product_array[$key]["active"] == '3') {
?>
